...
//    final Set<String> exemptPaths = Set.copyOf(securityProperties.getJwtExemptPaths());
//
    ...
    protected void doFilterInternal(HttpServletRequest request, @NotNull HttpServletResponse response, @NotNull FilterChain chain)
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith(BEARER_PREFIX)) {
            ...
            try {
                ...
                // Extract username or phone from token
                identifier = jwtUtil.extractUsername(jwt);
                logger.debug("JWT Token found in the header, identifier extracted: {}", identifier);

            } catch (JwtTokenException e) {
                logger.warn("Failed to extract identifier from JWT token: {}", e.getMessage());
                sendErrorResponse(response, HttpStatus.UNAUTHORIZED, e.getMessage());
                return;
            } catch (Exception e) {
                logger.warn("Unexpected error extracting identifier from JWT token: {}", e.getMessage());
                sendErrorResponse(response, HttpStatus.UNAUTHORIZED, "Invalid authentication token");
                return;
            }
        }
        ...
        if (identifier != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                // --- FIX: Check if token is temp token (profile completion) ---
                boolean isTempToken = false;
                try {
                    String purpose = jwtUtil.parseToken(jwt).get("purpose", String.class);
                    isTempToken = "profile_completion".equals(purpose);
                } catch (Exception ignored) { }

                if (isTempToken) {
                    // Don't load user yet, just allow request to pass for completing profile
                    UsernamePasswordAuthenticationToken auth =
                            new UsernamePasswordAuthenticationToken(null, null, null);
                    SecurityContextHolder.getContext().setAuthentication(auth);
                    logger.debug("Temp token detected for identifier: {}, skipping DB lookup", identifier);
                } else {
                    // Full JWT → load user from DB
                    UserDetails userDetails = this.authService.loadUserByUsername(identifier);
                    if (jwtUtil.validateToken(jwt, userDetails)) {
                        UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                                userDetails, null, userDetails.getAuthorities());
                        authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                        SecurityContextHolder.getContext().setAuthentication(authenticationToken);
                        logger.debug("JWT Token validated, authentication set for identifier: {}", identifier);
                    } else {
                        logger.warn("JWT Token validation failed for identifier: {}", identifier);
                        sendErrorResponse(response, HttpStatus.UNAUTHORIZED, "JWT token validation failed");
                        return;
                    }
                }
                ...
                logger.error("JWT token error for identifier: {}", identifier, e);
                sendErrorResponse(response, HttpStatus.UNAUTHORIZED, e.getMessage());
                return;
            } catch (Exception e) {
                logger.error("Error loading user details for JWT authentication: {}", identifier, e);
                sendErrorResponse(response, HttpStatus.UNAUTHORIZED, "Authentication failed");
                return;
            }
        }
